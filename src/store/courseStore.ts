...
import type { GraphData, Problem } from './graphStore';
...
export interface SavedCourse {
    ...
    updatedAt: string;
    graphData: GraphData;
    courseData: Partial<CourseModule>;
    problems: Problem[];
}
...
export const useCourseStore = create<CourseState>()(
    persist(
        (set, get) => ({
            ...
            saveCourse: (course) => set((state) => {
                const existingIndex = state.savedCourses.findIndex(c => c.id === course.id);
                if (existingIndex >= 0) {
