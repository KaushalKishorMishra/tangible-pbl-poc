	...
	<div className="group relative bg-white/70 backdrop-blur-xl rounded-[32px] border border-white/40 p-8 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-indigo-100/50 transition-all duration-500 hover:-translate-y-2 overflow-hidden">
		<div className="relative z-10">
			<div className="flex items-start justify-between mb-6">
				<div className={`p-4 bg-linear-to-br ${gradient} rounded-2xl text-white shadow-lg shadow-indigo-200 group-hover:scale-110 transition-transform duration-500`}>
					{icon}
				</div>
				{stats && (
			...
			<p className="text-slate-500 text-sm mb-8 font-medium leading-relaxed opacity-80">{description}</p>
			<button
				onClick={onClick}
		...
		<div className={`absolute -bottom-12 -right-12 w-40 h-40 rounded-full blur-3xl opacity-10 bg-linear-to-br ${gradient} group-hover:scale-150 transition-transform duration-700`}></div>
	</div>
);
...
const QuickStat: React.FC<QuickStatProps> = ({
	...
	<div className="bg-white/70 backdrop-blur-xl rounded-[28px] p-6 border border-white/40 shadow-lg shadow-slate-200/50 hover:shadow-xl transition-all duration-300 group">
		<div className="flex items-center justify-between">
			...
			<div className={`p-4 rounded-2xl ${color} text-white shadow-lg transition-transform group-hover:rotate-12 duration-300`}>
				{icon}
			</div>
		</div>
	</div>
					...
					<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
						<div className="animate-in fade-in slide-in-from-left-4 duration-700">
							...
							<p className="text-slate-500 mt-2 text-lg font-medium opacity-80">
								Your AI-powered design studio is ready. What will we build today?
							</p>
						</div>
			...
			<div className="max-w-7xl mx-auto px-8 py-12">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
					<QuickStat
						...
						change={savedCourses.length > 0 ? `+${savedCourses.length}` : undefined}
                ...
                <div id="saved-courses" className="mt-20 animate-in fade-in duration-1000 delay-300">
                    <div className="flex items-center justify-between mb-10">
						<div>
							...
							<p className="text-slate-500 font-medium mt-1 ml-6">Continue designing your learning architectures.</p>
						</div>
						<button onClick={handleViewCourses} className="text-indigo-600 font-black text-sm hover:underline flex items-center gap-1">
                    ...
                    {savedCourses.length > 0 ? (
                                    ...
                                    <div className="p-8">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mb-8">
                                            <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                                                ...
                                                <div>
													<p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Skills</p>
													<p className="text-sm font-black text-slate-900">{course.graphData.nodesCount}</p>
												</div>
                                            </div>
                                            <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                                                ...
                                                <div>
													<p className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Learners</p>
                        ...
                        <div className="bg-white/40 backdrop-blur-xl rounded-[40px] border-4 border-dashed border-slate-200 p-20 text-center animate-in zoom-in duration-700">
                            ...
                            <h3 className="text-2xl font-black text-slate-900 mb-3 tracking-tight">No courses yet</h3>
                            <p className="text-slate-500 mb-10 font-medium text-lg max-w-md mx-auto">Create your first AI-powered learning architecture to see it here.</p>
                            <button
                                onClick={handleCreateCourse}
					...
					<div className="bg-white/70 backdrop-blur-xl rounded-[32px] border border-white/40 shadow-xl shadow-slate-200/50 overflow-hidden">
						<div className="p-8">
							<div className="space-y-6">
								{[
									{ user: "Sarah Johnson", action: "completed the JavaScript Fundamentals module", time: "2 hours ago", color: "bg-emerald-500" },
									{ user: "New enrollment", action: "in Full-Stack Development Course", time: "4 hours ago", color: "bg-indigo-500" },
									{ user: "Mike Chen", action: "requested feedback on their React project", time: "6 hours ago", color: "bg-amber-500" }
								].map((activity, idx) => (
									<div key={idx} className="flex items-center gap-6 group">
										<div className={`w-3 h-3 ${activity.color} rounded-full shadow-lg shadow-current/20 group-hover:scale-150 transition-transform duration-300`}></div>
