...
export const ProblemSelectionPanel: React.FC<ProblemSelectionPanelProps> = ({
	...
	return (
		<div className="w-full h-full bg-gradient-to-br from-slate-50/50 to-indigo-50/30 p-8 overflow-y-auto scroll-smooth">
			<div className="max-w-6xl mx-auto">
				<div className="mb-12 text-center animate-in fade-in slide-in-from-top-4 duration-700">
					...
					<p className="text-slate-500 max-w-2xl mx-auto text-lg font-medium leading-relaxed">
						Select a real-world scenario to architect your curriculum. Each
						challenge is designed to build deep, practical competency.
					</p>
				</div>
					...
					{generatedProblems.map((problem, idx) => (
						...
						>
							<div className="p-10">
								...
								<h3 className="text-2xl font-black text-slate-900 mb-4 group-hover:text-indigo-600 transition-colors tracking-tight leading-tight">
									{problem.title}
								</h3>
								...
								<div className="flex items-center gap-4">
									<button
										className={`
                                            ...
                                            ${
																							selectedProblem?.id === problem.id
																								? "bg-gradient-to-br from-indigo-600 to-violet-700 text-white shadow-xl shadow-indigo-200"
							...
							<div
								className={`absolute -bottom-12 -right-12 w-40 h-40 rounded-full blur-3xl transition-all duration-500 opacity-20 ${
									selectedProblem?.id === problem.id
