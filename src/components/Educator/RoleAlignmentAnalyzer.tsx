...
interface JobRole {
  ...
  requiredSkills: {
    skillId: string;
    requiredLevel: 'awareness' | 'application' | 'mastery';
...
interface CourseCoverage {
  ...
  skills: {
    skillId: string;
    targetLevel: 'awareness' | 'application' | 'mastery';
  }[];
}
...
interface GapAnalysis {
  ...
  skillName: string;
  requiredLevel: 'awareness' | 'application' | 'mastery';
  courseLevel: 'awareness' | 'application' | 'mastery' | null;
...
export const RoleAlignmentAnalyzer: React.FC<RoleAlignmentAnalyzerProps> = ({
  ...
  const gapAnalysis = useMemo(() => {
    ...
    return selectedRoleData.requiredSkills.map(requiredSkill => {
      ...
      if (courseLevel) {
        const levelOrder = { awareness: 1, application: 2, mastery: 3 };
