...
const PRICING = {
	...
	default: { input: 0.15, output: 0.6 }, // Fallback to mini pricing
};
...
export const TokenUsageBadge: React.FC = () => {
	...
	const stats = usageHistory.reduce(
		(acc, usage) => {
	...
	return (
		...
		>
			<div className="flex items-center gap-3 px-4 py-2 bg-white/50 backdrop-blur-md rounded-2xl border border-white/40 shadow-sm hover:shadow-indigo-100/50 hover:border-indigo-200 transition-all cursor-help group">
						...
						<div className="bg-gradient-to-br from-indigo-600 to-violet-700 p-6 text-white relative overflow-hidden">
							<div className="relative z-10">
								<div className="flex justify-between items-start mb-4">
									<h3 className="text-[10px] font-black opacity-70 uppercase tracking-[0.3em]">
										AI Synthesis Cost
									</h3>
								...
								<div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
									{usageHistory.slice(0, 5).map((usage, i) => {
										...
										return (
											<div
												key={i}
												className="flex items-center justify-between p-3 hover:bg-white rounded-2xl transition-all border border-transparent hover:border-slate-100 hover:shadow-sm group/row"
											>
												<div className="flex flex-col">
													<span className="text-[10px] font-black text-slate-700 truncate max-w-[120px] uppercase tracking-wider">
														{usage.model?.split("-").slice(0, 2).join(" ") ||
															"GPT 4o Mini"}
													</span>
